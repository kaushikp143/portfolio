import { Cpu, Award } from 'lucide-react';

const GithubIcon = ({ className }) => (
  <svg
    className={className}
    viewBox="0 0 24 24"
    fill="none"
    stroke="currentColor"
    strokeWidth="2"
    strokeLinecap="round"
    strokeLinejoin="round"
  >
    <path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4" />
    <path d="M9 18c-4.51 2-5-2-7-2" />
  </svg>
);

const LinkedinIcon = ({ className }) => (
  <svg
    className={className}
    viewBox="0 0 24 24"
    fill="none"
    stroke="currentColor"
    strokeWidth="2"
    strokeLinecap="round"
    strokeLinejoin="round"
  >
    <path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z" />
    <rect width="4" h="12" x="2" y="9" />
    <circle cx="4" cy="4" r="2" />
  </svg>
);

export default function Profiles() {
  const profiles = [
    {
      name: 'GitHub',
      icon: <GithubIcon className="w-8 h-8 text-[#14b8a6]" />,
      username: '@aaravsharma',
      desc: 'Source code for embedded firmware, ROS2 control libraries, and React frontend monitoring dashboards.',
      link: 'https://github.com/aaravsharma',
    },
    {
      name: 'LinkedIn',
      icon: <LinkedinIcon className="w-8 h-8 text-[#c084fc]" />,
      username: 'aarav-sharma-ece',
      desc: 'Connect for internship opportunities, research collaborations, and professional updates in embedded IoT.',
      link: 'https://linkedin.com/in/aarav-sharma-ece',
    },
    {
      name: 'Hackster.io',
      icon: <Cpu className="w-8 h-8 text-amber-400" />,
      username: 'aarav-ece-hacks',
      desc: 'Complete system build walkthroughs, wiring schematics, and component lists for hardware creations.',
      link: 'https://hackster.io/aarav-ece-hacks',
    },
    {
      name: 'LeetCode',
      icon: <Award className="w-8 h-8 text-orange-400" />,
      username: 'aarav_sharma_99',
      desc: '350+ programming problems solved. Focus on memory optimization and algorithmic efficiency in C++ & JS.',
      link: 'https://leetcode.com/aarav_sharma_99',
    },
  ];

  return (
    <section id="profiles" className="py-24 px-6 md:px-12 relative overflow-hidden bg-slate-950/25">
      <div className="max-w-7xl mx-auto">
        {/* Section Heading */}
        <div className="text-left mb-16">
          <p className="text-sm font-mono text-[#14b8a6] tracking-wider uppercase mb-2">&gt; Connections</p>
          <h2 className="text-4xl md:text-5xl font-bold text-white tracking-tight">Developer Profiles</h2>
          <div className="w-20 h-1 bg-[#14b8a6] mt-4 rounded-full" />
        </div>

        {/* Profiles Grid */}
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          {profiles.map((profile, idx) => (
            <a
              key={idx}
              href={profile.link}
              target="_blank"
              rel="noopener noreferrer"
              className="group bg-slate-900/30 backdrop-blur-sm border border-slate-800/60 hover:border-slate-700/80 p-6 rounded-3xl text-left block hover:scale-[1.02] hover:bg-slate-900/40 transition-all duration-300 shadow-md"
              onClick={(e) => {
                // Keep simulated links within context
                e.stopPropagation();
              }}
            >
              {/* Profile Icon */}
              <div className="mb-6 flex justify-between items-center">
                <div className="p-3 bg-slate-950/80 border border-slate-800 rounded-2xl group-hover:border-slate-600 transition-colors">
                  {profile.icon}
                </div>
              </div>

              {/* Title & Username */}
              <h3 className="text-xl font-bold text-slate-100 group-hover:text-[#14b8a6] transition-colors">
                {profile.name}
              </h3>
              <p className="text-xs font-mono text-slate-400 mt-1 mb-4">
                {profile.username}
              </p>

              {/* Description */}
              <p className="text-sm text-slate-400 leading-relaxed">
                {profile.desc}
              </p>
            </a>
          ))}
        </div>
      </div>
    </section>
  );
}
